...
class CreateNode(Node):
    ...
    async def create(self): # TODO: Add Permission Support, emoji support
        ...
        print(self.interpreter.get_next_token(), "eeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
        to_create = self.interpreter.expect_token("KEYWORD:channel|KEYWORD:role|KEYWORD:emoji|KEYWORD:attachment").value
        self.interpreter.expect_token("KEYWORD:named")
        name = (await ExpressionNode(self.interpreter.next_token()).create()).value
        if to_create != "attachment":
            ...
            path = (await ExpressionNode(self.interpreter.next_token()).create()).value
            if type(path) == str:
                file = discord.File(path,filename=name)
                self.value = AttachmentClass(file)
            else:
                self.interpreter.error("InvalidPath",f"Path {path} is not a string")
